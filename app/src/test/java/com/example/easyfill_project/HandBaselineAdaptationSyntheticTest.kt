...
/**
 * Synthetic test for accumulated hand-baseline adaptation.
 *
 * It uses the production MotionAnalyzer, TremorSpectrumAnalyzer,
 * baseline models and HandTremorEvaluator.
 *
 * The Welford merge is reproduced here because the production
 * merge function is private inside MotionTrackingController.
 *
 * Output:
 * app/build/synthetic-hand-baseline-adaptation/
 * - synthetic_hand_baseline_adaptation.csv
 * - synthetic_hand_baseline_adaptation_summary.txt
 */
class HandBaselineAdaptationSyntheticTest {

    @Test
    fun generateHandBaselineAdaptationDataset() {
    ...
    private fun summary(rows: List<Row>): String {
        ...
        return """
        Synthetic hand baseline-adaptation evaluation completed.
    ...
    private fun outputDirectory(): File {
        ...
        return File(
            app,
            "build/synthetic-hand-baseline-adaptation"
        ).apply {
            check(exists() || mkdirs())
        }
    }
    ...
    companion object {
        ...
        private const val ACCEPTED_USE_COUNT = 5
